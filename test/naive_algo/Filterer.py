...
class Filterer():
    ...
    def join_xml(self, join_rdb_results):
        start_join_xml = timeit.default_timer()
        matches = []
        for e in join_rdb_results[0]:
            if not matches:
                for join_rslt in join_rdb_results:
    ...
    def perform(self):
        ...
        # join_results = self.join_rdb_naive()

        join_results = self.join_rdb_pandas()

        self.join_xml_pandas(join_results)

        print('TOTAL TIME: ', timeit.default_timer() - start_perform)