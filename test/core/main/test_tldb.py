...
class TestTLDB(TestCaseCompare):
    ...
    def test_rtree_index_csv_file(self):
        ...
        input_folder = root_path() / 'test' / 'io' / 'in' / 'cases' / 'simple_small' / 'A_B_D_table.dat'
    ...
    def test_rtree_index_xml_file(self):
        method_id = self.id().split('.')[-1]
        self.tldb.load_object_from_xml('xml', root_path().joinpath('test/io/in/core/lib/dewey_id/messages.xml'),
                                       max_n_children=2)
        with self.out_file[method_id].open(mode='w') as f:
    ...
    def test_rtree_index_from_folder(self):
        ...
        tldb.load_from_folder(root_path() / 'test' / 'io' / 'in' / 'cases' / 'simple_small', max_n_children=2)
        with self.out_file[method_id].open(mode='w') as f:
