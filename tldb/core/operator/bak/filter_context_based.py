...
from tldb.core.lib.boundary import value_boundaries_union, value_boundary_intersection
...
class Filterer(Operator):
    ...
    def init_link(self, flt_node: XMLNode):
        ...
        def init_link_sql():
            ...
            for table in tables:
                ...
                _l.verbose('\t'+ 'Checking table: ' + table)
                ...
                if not join_boundaries:
                    ...
                    nodes_in_range = nodes_range_search(starters, boundaries)
                    for node in nodes_in_range:
                        updated_join_b.append(dict(zip(table_e, node.boundary)))
                        updated_table_nodes.add(node)

                    _l.verbose('\t' * 3 + 'First table, nodes satisfy flt_node v_boundary: '
                    ...
                    for join_b in join_boundaries:
                        ...
                        _l.verbose('\t' * 3 + 'Nodes in range join_b: ' + str(join_b) + ': ' +
                                   str([node.boundary for node in nodes_in_range]))
                ...
                _l.verbose('\t' * 2 + 'Link ' + table + ' nodes: ' +
                           str([node.boundary for node in link_sql[table]]))
                _l.verbose('\t' * 3 + 'Updated join_b: ' +
        ...
        def init_link_xml():
            ...
            for c_e in child_elements:
                ...
                _l.verbose('\t' * 3 + 'Linked nodes: ' + str(c_e_nodes))
        ...
        _l.debug('\t'+ 'Init link sql Results: ' + str(success))
        ...
        _l.debug('\t'+ 'Init link children Results: ' + str(success))
        _l.debug('\t'+ 'Link Children: ' + str(flt_node.link_children))
        if not success:
            flt_node.timer['init_link'][1] = timeit.default_timer()
            log_node_timer(flt_node, _l.timer, f_n_idx)
            return False
        ...
        _l.debug('\t'+ 'Init link xml Results: ' + str(success))
    ...
    def filter_with_context(self, flt_node: XMLNode, contexts: List[Context]):
        ...
        # f_n_idx = self.elements.index(flt_node.name)
        _l.debug('\t'+ str(len(contexts)) + ' contexts')
        ...
        _l.timer('%s %.3f', '\t'+ 'Filter ' + str(ori_len_contexts) + '->' + str(len(contexts)) +
                 ' contexts based on node v took: ', timeit.default_timer() - start_flt_w_c)

        if not contexts:
            _l.debug('\t'+ 'None of contexts interX with node v_boundary')
            _l.timer('%s %.3f', '\t'+ 'Filter with context took: ', timeit.default_timer() - start_flt_w_c)
            _l.debug('\n')
            return False, None
        ...
        if not flt_node.inited_link:
            ...
            if not success:
                _l.debug('\t'+ 'Node filtered after init link')
                _l.timer('%s %.3f', '\t'+ 'Filter with context took: ', timeit.default_timer() - start_flt_w_c)
                _l.debug('\n')
                return False, None
        ...
        _l.timer('%s %.3f', '\t'+ 'Filter ' + str(len(contexts)) + '->' + str(len(remaining_contexts)) +
                 ' contexts based on node join boundaries took: ', timeit.default_timer() - start_flt_w_c)
        contexts = remaining_contexts

        _l.verbose('\t'+ 'Contexts updated base on join_boundaries: ' + str(contexts))
        if not remaining_contexts:
            _l.debug('\t'+ 'None of contexts interX with node join boundaries')
            _l.timer('%s %.3f', '\t'+ 'Filter with context took: ', timeit.default_timer() - start_flt_w_c)
            _l.debug('\n')
            return False, None
        ...
        for c_e in child_elements:
            updated_contexts = []
            _l.verbose('\t' * 2 + 'Checking element ' + c_e)
            for node in flt_node.link_xml[c_e]:
                _l.verbose('\t' * 3 + 'Traverse to ' + str(node))
                if node.filtered:
                    _l.verbose('\t' * 4 + 'Already Filtered')
                    continue
                success, updated_n_c = self.filter_with_context(node, [copy.copy(context) for context in contexts])
                _l.verbose('\t' * 4 + 'Returned contexts of ' + str(node) + ': ' + str(updated_n_c))
                if success:
                    for context in updated_n_c:
                        updated_contexts.append(context)

            _l.verbose('\t' * 2 + 'Updated context after ' + c_e + ': ' + str(updated_contexts))
            _l.timer('%s %.3f', '\t'+ 'Filter ' + str(len(contexts)) + '->' + str(len(updated_contexts)) +
