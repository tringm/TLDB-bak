...
from tldb.core.main.structure.node import *
from .index_structure import IndexStructure


class RTree(IndexStructure):
    def __init__(self, name):
        super().__init__(name, 'rtree')
        self.root = None

    def load(self, entries, method='str', dimension=1, max_n_children=2):
        logger = logging.getLogger(f"Indexer:{self.type}")
        logger.debug(f"Start loading {self.name} with method {method}")
        if method == 'str':
            self.str_bulk_loading(Node, entries, max_n_children, dimension)
        elif method == 'stripe':
            self.stripe_bulk_loading(Node, entries, max_n_children, dimension)
        else:
    ...
    def str_bulk_loading(self, node_type, entries: [Entry], max_n_children: int, dimension: int):
        ...
        def entries_to_leaf_nodes(some_entries: [Entry]):
            ...
            node = node_type(max_n_children, entries=some_entries, name=self.name, dimension=dimension)
        ...
        def child_nodes_to_parent_node(child_nodes: [Node]):
            ...
            node = node_type(max_n_children, children=child_nodes, name=self.name, dimension=dimension)
