from src.RTree import XMLRTree, SQLRTree
...
def build_RTree_XML(folder_name, element_name, max_n_children, dimension=1):
    ...
    """
    entries = XMLRTree.load(folder_name, element_name)
    print('Loaded ', len(entries), 'entries')
    # print(entries[0].coordinates)
    root = XMLRTree.bulk_loading(entries, element_name, max_n_children, dimension)
...
def build_RTree_SQL(folder_name, file_name, all_elements_name, max_n_children):
    ...
    """
    entries = SQLRTree.load(folder_name, file_name)
    ...
    root = SQLRTree.bulk_loading(entries, table_name, max_n_children, dimension)
    return root
